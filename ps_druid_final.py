...
# print(response.text)
res = response.json()
spark = SparkSession.builder.appName("ps_druid2").getOrCreate()
        ...
        StructField("events", ArrayType(
            StructType(
                [
                    ...
                    StructField("section_progress", StringType(), False),
...
df1 = df.select(df["events"]["Course_name"].alias("Title of the Course"),
               df["events"]["batch_name"].alias("Names of the Batches created for the course"),
               df["events"]["User_FirstName"].alias("Full Name of the Learners in the batch"),
               df["events"]["course_progress"].alias("Course Completion Percentage for each learner in the batch"),
...
df3 = df1.join(df2,[df1["Title of the Course"]==df2["events[Course_name]"],df1["Name of the resources in the course"]==df2["events[section_name]"],df1["User_id"]==df2["events[User_id]"],df1["Course Completion Percentage for each learner in the batch"]==df2["max(events[course_progress])"]],"inner").select(df1['*'],df2["max(events[course_progress])"])
...
df4 =df4.withColumnRenamed("max(events[course_progress])","Course Completion Percentage for each learner in the batch")
df4 =df4.withColumnRenamed("max(Time stamp of completing the course)","Time stamp of completing the course")
# df4.printSchema()
df4.show(100)

df4.coalesce(1).write.format('com.databricks.spark.csv').mode('overwrite').save('Batch-wise Partner Information',header = 'true')