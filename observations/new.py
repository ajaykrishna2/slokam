...
obs_sub_dfn1=obs_sub_dfn.withColumn(
   "exploded_userProfile",F.explode_outer(F.col('userLocations'))
)
...
entities_df = melt(obs_sub_dfn1,
        id_vars=["exploded_userProfile.name","exploded_userProfile.id"],
        value_vars=["exploded_userProfile.type"]
    ).select("name","value","id").dropDuplicates()
# print(entities_df.value.show())
entities_df = entities_df.withColumn("variable",F.concat(F.col("value"),F.lit("_externalId")))
print(entities_df)

# entities_df = entities_df.withColumn("variable",F.concat(F.col("entityType"),F.lit("_externalId")))
# print(entities_df.take(1))
entities_df.withColumn("flag",F.regexp_replace(F.col("variable"),"Id","Name"))
print(entities_df)
# entities_df= entities_df.groupBy(["value"]).agg(first(F.col("name")))
                               # .pivot("flag").agg(first(F.col("exploded_userProfile.name")))
# entities_df.show()
