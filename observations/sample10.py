...
for i in pr_id:
    ...
    obs_sol_cursorMongo = solutionCollec.aggregate(
       [
          ...
          {"$project": {"_id": {"$toString": "$_id"}, "programId": {"$toString": "$programId"}}},
          {"$match": {"programId": f'''{i}'''}}
       ]
    )
    ...
    obs_soln_df.show(truncate=False)
...
for k in flat_list:
    ...
    obs_sub_df1.show()
    # print(obs_sub_df1.select(obs_sub_df1["answers"]).show(truncate=False))
    ...
    obs_sub_df_melt = melt(obs_sub_df,
            id_vars=["status","entity_externalId","entity_id","entity_type","user_id","solution_id",
                ...
                "updatedAt","role_title","channel","parent_channel","board_name","organisation_name","organisation_id","answers",'completedDate'],
    ...
    obs_sub_df2=obs_sub_df1.select("submission_id","value", obs_sub_df1.value['qid'].alias('qid'), obs_sub_df1.value['responseType'].alias('responseType'),obs_sub_df1.value['payload'].alias('payload'))
    ...
    obs_sub_dfn1.show()
    obs_sub_dfn1 = obs_sub_dfn1.select('submission_id', 'status', 'entity_externalId', 'entity_id', 'entity_type', 'user_id', 'solution_id', 'solution_externa', 'entity_name', 'completedDate', 'program_id', 'program_externalId', 'app_name', 'private_program', 'solution_type', 'ecm_marked_na', 'updatedAt', 'role_title', 'state_externalId', 'block_externalId', 'district_externalId', 'cluster_externalId', 'school_externalId', 'channel', 'userName', 'userId', 'answers', 'parent_channel', 'board_name', 'organisation_name', 'organisation_id',  'qid', 'responseType',  'question', 'labels','completedDate',"state_name","block_name","district_name","cluster_name","school_name")
    ...
    # obs_sub_df12 = obs_sub_df12.withColumn('evidences', lit(None).cast(StringType()))
    obs_sub_df12 = obs_sub_df12.select('submission_id', 'status', 'entity_externalId', 'entity_id', 'entity_type','user_id', 'solution_id', 'solution_externalId', 'entity_name', 'completedDate','program_id', 'program_externalId', 'app_name', 'private_program','solution_type', 'ecm_marked_na', 'updatedAt', 'role_title', 'state_externalId','block_externalId', 'district_externalId', 'cluster_externalId','school_externalId', 'channel', 'userName', 'userId', 'answers','parent_channel', 'board_name', 'organisation_name', 'organisation_id', 'qid','responseType', 'question', 'labels', 'completedDate',"state_name","block_name","district_name","cluster_name","school_name")
    obs_sub_df=obs_sub_dfn1.union(obs_sub_df12)

    obs_sub_df= obs_sub_df.withColumnRenamed("solution_externalId", "solution_name").withColumnRenamed("program_externalId", "program_name")
    output_dfs.append(obs_sub_df)

df_output = functools.reduce(DataFrame.union, output_dfs)
df_output=df_output.select('submission_id', 'status', 'entity_externalId', 'entity_id', 'entity_type','user_id', 'solution_id', 'solution_name', 'entity_name', 'completedDate','program_id', 'program_name', 'app_name', 'private_program','solution_type', 'ecm_marked_na', 'updatedAt', 'role_title', 'state_externalId','block_externalId', 'district_externalId', 'cluster_externalId','school_externalId', 'channel', 'userName', 'userId', 'parent_channel', 'board_name', 'organisation_name', 'organisation_id', 'qid','responseType', 'question', 'labels',"state_name","block_name","district_name","cluster_name","school_name")


row_list1=df_output.select("program_id").collect()
