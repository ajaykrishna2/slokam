            ...
            try:
                if obSub["userProfile"]:
                    if "userLocations" in obSub["userProfile"] and obSub["userProfile"]["userLocations"]:
                        userLocations = obSub["userProfile"]["userLocations"]
                        for entInf in userLocations:
                            if entInf['type'] == 'state':
                                stateName = entInf['name']
                                stateExternalId =entInf['code']
                                # print(stateName)
                            if entInf["type"] == "block":
                                blockName = entInf['name']
                                blockExternalId = entInf['code']
                                # print(blockName)
                            if entInf["type"] == "district":
                                districtName = entInf['name']
                                districtExternalId = entInf['code']
                                # print(districtExternalId)
                            if entInf["type"] == "cluster":
                                clusterName = entInf['name']
                                clusterExternalId = entInf['code']
                                # print(clusterExternalId)
                            if entInf["type"] == "school":
                                clusterName = entInf['name']
                                userSchoolUDISE = entInf["code"]
                                userSchool = str(entInf["id"])
                                # print(clusterExternalId)
                                # print(userSchool)
                ...
                # if 'userProfile' in obSub :
                #     if 'locations' in obSub['userProfile']:
                #         print(obSub['userProfile']['locations'])
                    # print(obSub['userProfile'])
            except KeyError:
                pass
                # if 'locations' in obSub['userProfile']:
                #     print(obSub['userProfile']['locations'])
                                            ...
                                            if userEntityRelatedResultKeyCheck == True:
                                                if userEntityRelated["result"]:
                                                    if (userEntityRelated["result"]["entityType"] == "district") or (
                                                            ...
                                                            userEntityRelated["result"]["entityType"] == "cluster") or (
                                                        ...
                                                        roleObj[
                                                            userEntityRelated["result"]["entityType"] + '_externalId'] = \
                                                        userEntityRelated["result"]["metaInformation"]["externalId"]
                                                    ...
                                                    for usrEntityData in userEntityRelated["result"]["relatedEntities"]:
                                                        if (usrEntityData['entityType'] == "district") or (
                                                            ...
                                                            roleObj['user_' + usrEntityData['entityType'] + 'Name'] = \
            ...
            if 'answers' in obSub.keys():
                ...
                for ans in answersArr:
                    ...
                    def creatingObj(
                        ...
                        try:
                            if 'program' in obSub:
                                observationSubQuestionsObj['programName'] = obSub['name']
                                observationSubQuestionsObj['programDescription'] = obSub['description']
                        ...
                        #     solutionObj = solu
                        try:
                            if 'solutions' in obSub:
                                observationSubQuestionsObj['solutionName'] = obSub['name']
                                observationSubQuestionsObj['scoringSystem'] = obSub['scoringSystem']
                                observationSubQuestionsObj['solutionDescription'] = obSub['description']
                                        ...
                                        if obj['type'] == 'criteria':
                                            if (str(obj['externalId']) == str(answer['criteriaId'])):
                                                ...
                                                if type(obj['externalId']) != str:
                                                    for cri in criteriaCollec.find(
                                                            {'_id': ObjectId(str(obj['externalId']))}):
