...
cconfig_path = os.path.split(os.path.dirname(os.path.abspath(__file__)))
...
surveySubmissionsCollec = db[config.get('MONGO', 'observation_sub_collection')]

solutionsCollec = db[config.get('MONGO', 'solutions_collection')]
surveyCollec = db[config.get('MONGO', 'observation_collection')]
...
def CollectingTheUserData(data):
    '''
    ...
    if data is not None:
        ...
        # Get user Sub type and district
        if data["userRoleInformation"]:
            try:
                output["user_subtype"] = data["userRoleInformation"]["role"]
                output["district"] = data["userRoleInformation"]["district"]
            except KeyError:
                pass
        ...
        if data["status"]:
            try:
                output["status_of_submission"] = data["status"]
            except KeyError:
                pass

        # Get the Submission date
        if data["evidencesStatus"]:
            try:
                output["submission_date"] = data["evidencesStatus"]["submissions"]["submissionDate"]
            except KeyError:
                pass
        ...
        if data["criteria"]:
            try:
                output["created_date"] = data["criteria"]["createdAt"]
            except KeyError:
                pass

        # Get user profile type
        if data["userProfile"]["profileUserTypes"]:
            try:
                temp_userType = set([types["type"] for types in data["userProfile"]["profileUserTypes"]])
                output["user_type"] = ", ".join(temp_userType)
            except KeyError:
                pass

        # Get locations
        if data["userProfile"]["userLocations"]:
            for loc in data["userProfile"]["userLocations"]:
                # Get block
                try:
                    if loc["type"] == "block":
                        output["block_id"] = loc["id"]
                        output["block_name"] = loc["name"]
                        output["block_externalId"] = loc["code"]
                except KeyError:
                    pass
                # Get State
                try:
                    if loc["type"] == "state":
                        output["state_id"] = loc["id"]
                        output["state_name"] = loc["name"]
                        output["state_externalId"] = loc["code"]
                except KeyError:
                    pass
                # Get District
                try:
                    if loc["type"] == "district":
                        output["district_id"] = loc["id"]
                        output["district_name"] = loc["name"]
                        output["district_externalId"] = loc["code"]
                except KeyError:
                    pass
                # Get school
                try:
                    if loc["type"] == "school":
                        output["school_id"] = loc["id"]
                        output["school_name"] = loc["name"]
                except KeyError:
                    pass

        # Get board
        if data["userProfile"]["framework"]["board"]:
            try:
                output["board"] = ",".join(data["userProfile"]["framework"]["board"])
            except KeyError:
                pass
    return output
                ...
                if 'answers' in obSub.keys():
                    ...
                    for ans in answersArr:
                        ...
                        def creatingObj(answer, quesexternalId, ans_val, instNumber, responseLabel):
                            ...
                            surveySubQuestionsObj.update(CollectingTheUserData(obSub))
...
for msg_data in surveySubmissionsCollec.find().limit(10):
    # print("aaaaaa")
    # print(msg_data)
    obj_creation(msg_data["_id"])
