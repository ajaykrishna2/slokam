...
def final_projects_tasks_distinctCnt_df_byProgramlevel():
    # projects submission distinct count by program level
    final_projects_tasks_distinctCnt_df_byProgramlevel = final_projects_df.groupBy("program_name", "program_id","status_of_project", "state_name","state_externalId","private_program","project_created_type","parent_channel").agg(countDistinct(F.col("project_id")).alias("unique_projects"),countDistinct(F.col("createdBy")).alias("unique_users"),countDistinct(when(F.col("task_evidence_status") == "true", True), F.col("project_id")).alias("no_of_imp_with_evidence"))
    final_projects_tasks_distinctCnt_df_byProgramlevel = final_projects_tasks_distinctCnt_df_byProgramlevel.withColumn("time_stamp", current_timestamp())
    final_projects_tasks_distinctCnt_df_byProgramlevel = final_projects_tasks_distinctCnt_df_byProgramlevel.dropDuplicates()
    final_projects_tasks_distinctCnt_df_byProgramlevel.coalesce(1).write.format("json").mode("overwrite").save(
        config.get("OUTPUT_DIR", "projects_distinctCount_byProgramlevel") + "/"
    )
    final_projects_df.unpersist()
    final_projects_tasks_distinctCnt_df_byProgramlevel.unpersist()
...
final_projects_tasks_distinctCnt_df_byProgramlevel()
#projects submission distinct count by program level
for filename in os.listdir(config.get("OUTPUT_DIR", "projects_distinctCount_byProgramlevel")+"/"):
    if filename.endswith(".json"):
       os.rename(
           config.get("OUTPUT_DIR", "projects_distinctCount_byProgramlevel") + "/" + filename,
           config.get("OUTPUT_DIR", "projects_distinctCount_byProgramlevel") + "/ml_projects_distinctCount_byProgramlevel.json"
        )
...
# #projects submission distinct count by program level
# for files in os.listdir(local_distinctCnt_path):
#     if "ml_projects_distinctCount_byProgramlevel.json" in files:
#         blob_service_client.create_blob_from_path(
#             container_name,
#             os.path.join(blob_distinctCnt_byProgramlevel_path,files),
#             local_distinctCnt_byProgramlevel_path + "/" + files
#         )
...
# os.remove(config.get("OUTPUT_DIR", "projects_distinctCount_byProgramlevel") + "/ml_projects_distinctCount_byProgramlevel.json")
#
...
#projects submission distinct count by program level
# ml_distinctCnt_projects_byProgramlevel_spec = json.loads(config.get("DRUID","ml_distinctCnt_projects_byProgramlevel_status_spec"))
# ml_distinctCnt_projects_byProgramlevel_datasource = ml_distinctCnt_projects_byProgramlevel_spec["spec"]["dataSchema"]["dataSource"]
# distinctCnt_projectsby_Programlevel_start_supervisor = requests.post(druid_batch_end_point, data=json.dumps(ml_distinctCnt_projects_byProgramlevel_spec), headers=headers)
# if distinctCnt_projects_start_supervisor.status_code == 200:
#    successLogger.debug(
#         "started the batch ingestion task sucessfully for the datasource " + ml_distinctCnt_projects_byProgramlevel_datasource
#    )
#    time.sleep(50)
# else:
#    errorLogger.error(
#         "failed to start batch ingestion task" + str(distinctCnt_projects_byProgramlevel_start_supervisor.status_code)
#    )
#    errorLogger.error(distinctCnt_projects_byProgramlevel_start_supervisor.json())
dimensionsArr = []
