...
for i in pr_id:
    ...
    obs_sub_schema = StructType(
        [
            ...
            StructField("evidencesStatus", ArrayType(StructType([
                StructField("submissions", ArrayType(StructType([StructField('submittedByName', StringType(), True)
                                                                ])))]))),
            ...
            # StructField("evidences", ArrayType(StructType([
            #     StructField("submissions", ArrayType(StructType([StructField('submittedByName', StringType(), True)
            #                                                      ])))]))),
            # StructField('solutionId', StringType(), True),
            # StructField("evidences",StructType([
            #     StructField("submissions",StructType([
            #     StructField("answers", ArrayType(StructType([
            #     StructField("fileName",ArrayType(StructType([StructField("sourcePath", StringType(), True)])),True)])))]))])),
            # StructField("evidences",MapType(StringType(),ArrayType(StructType([StructField("submissions",StructType([
            #     StructField("answers",StringType(),True)]))])))),
            StructField('evidences',StringType(),True),
            # StructField(
            #    'evidences',
            #     ArrayType(StructField(['submissions',ArrayType(StructType([StructField('fileName', ArrayType(StructType([StructField("sourcePath", StringType(), True)])))]))]))),
    ...
    obs_sub_df = obs_sub_df1.select(
        "status",
        obs_sub_df1["evidencesStatus"]["submissions"].alias("userName"),
        ...
        obs_sub_df1["evidences"].alias("evidences"),
        "completedDate",
        ...
        # element_at(col('evidences.submissions.answers.fileName'), -1).alias("evidences")
    )
    # obs_sub_df = obs_sub_df.withColumn('evidences', col('evidences').cast('Array'))
    obs_data = obs_sub_df.select("userName", explode('userName'))
    # obs_data1= obs_sub_df.select("evidences", explode('evidences'))
    # obs_data1.show()
    obs_sub_df.show()
    ...
    obs_sub_df_melt = melt(obs_sub_df,
                           id_vars=["status", "solution_id", "userName", "user_id", "solution_externalId",
                                    "submission_id", "completedDate", "program_id", "program_externalId", "board_name",
                                    "organisation_name", "organisation_id", "answers","private_program","evidences"],
                           value_vars=["state_externalId", "block_externalId", "district_externalId",
                                       "cluster_externalId", "school_externalId"]
                           )
    ...
    obs_ent_sub_df_melt = obs_ent_sub_df_melt.withColumn("flag",
    ...
    obs_sub_df2 = obs_sub_df1.select("submission_id", "value", obs_sub_df1.value['qid'].alias('qid'),
                                     obs_sub_df1.value['responseType'].alias('responseType'),
                                     obs_sub_df1.value['gpsLocation'].alias('gpsLocation'),
                                     obs_sub_df1.value['payload'].alias('payload'))
    ...
    # obs_sub_df3.show()
    obs_sub_df3 = obs_sub_df2.select("submission_id", "payload", obs_sub_df2.payload['question'].alias('question'),
                                     obs_sub_df2.payload['labels'].alias('labels'))
    ...
    obs_sub_dfn1 = obs_sub_dfn1.select("solution_id", "userName", "user_id", "solution_name", "submission_id",
                                       "completedDate", "program_id", "program_name", "board_name", "organisation_name",
                                       "organisation_id", "answers", 'qid', 'responseType', 'question', 'labels',
                                       'completedDate', "state_name", "block_name", "district_name", "cluster_name",
                                       "school_name", 'state_externalId', 'block_externalId', 'district_externalId',
                                       'cluster_externalId', 'school_externalId', 'gpsLocation','private_program',"evidences")
    ...
    obs_sub_df12 = obs_sub_df12.select("solution_id", "userName", "user_id", "solution_name", "submission_id",
                                       "completedDate", "program_id", "program_name", "board_name", "organisation_name",
                                       "organisation_id", "answers", 'qid', 'responseType', 'question', 'labels',
                                       'completedDate', "state_name", "block_name", "district_name", "cluster_name",
                                       "school_name", 'state_externalId', 'block_externalId', 'district_externalId',
...
df_output = df_output.select("solution_id", "userName", "user_id", "solution_name", "submission_id", "completedDate",
                             "program_id", "program_name", "board_name", 'qid', 'responseType', 'question', 'labels',
                             "state_name", "block_name", "district_name", "cluster_name", "school_name",
                             'school_externalId', 'gpsLocation','private_program',"evidences")
df_output=df_output.withColumn('userName', col('userName').cast('string'))
df_output=df_output.withColumn("userName", F.regexp_replace("userName","\[","")).withColumn("userName", F.regexp_replace("userName","\]","")).withColumn("question", F.regexp_replace("question","\[","")).withColumn("labels", F.regexp_replace("labels","\[","")).withColumn("question", F.regexp_replace("question","\]","")).withColumn("labels", F.regexp_replace("labels","\]",""))
# df_output=df_output.replace(float('nan'), None)
# df_output.show()
daaaa = df_output.filter(df_output["evidences"].contains("fileName=[]"))
daaaa.show()
df_output = df_output.withColumn("gpsLocation",
                   when(col("gpsLocation") != '', col("gpsLocation")).otherwise(None))
df_output = df_output.na.fill({"gpsLocation": 'gps location not found for school'})
# df_output = df_output.na.fill(value='gps location not found for school', subset=["gpsLocation"])
...
for i in output:
    ...
    for j in range(len(df_final1)):
        ...
        for i in range(len(df_final)):
            ...
            df_finall.withColumn('userName', col('userName').cast('string')).coalesce(1).write.format("csv").option("header", True).mode("overwrite").save(
