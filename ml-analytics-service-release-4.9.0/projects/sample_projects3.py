...
# projects_df_cols.withColumn("userProfile", explode(array($"userProfile.*"))).show(false)
obs_sub_dfn1=projects_df_cols.withColumn(
   "exploded_userProfile",F.explode_outer(F.col('userProfile'))
)

# obs_sub_dfn1.select("project_id","exploded_userProfile.*").show()
# obs_sub_dfn1.select("exploded_userProfile").show(truncate=False)
...
entities_df = entities_df.withColumn("variable2",F.concat(F.col("type"),F.lit("_id")))

ds=entities_df.groupBy("project_id").pivot("variable").agg(first("id"))

ds1=entities_df.groupBy("project_id").pivot("variable1").agg(first("name"))

ds2=entities_df.groupBy("project_id").pivot("variable2").agg(first("value"))

dfff=ds.join(ds1,["project_id"],how='outer')
dffff=dfff.join(ds2,["project_id"],how='outer')
print(dffff.count())
dffff.show()
dffff=dffff.drop('null')
print(projects_df_cols.columns)
print(dffff.columns)
print(projects_df_cols.count())
entities_df.unpersist()
dffff.unpersist()
projects_df_cols=projects_df_cols["project_id", "project_created_type", "project_title", "project_title_editable", "program_id", "program_externalId", "program_name", "project_duration", "project_last_sync", "project_updated_date", "project_deleted_flag", "area_of_improvement", "status_of_project", "createdBy", "project_description", "project_goal", "parent_channel", "project_created_date", "task_id", "tasks", "task_assigned_to", "task_start_date", "task_end_date", "tasks_date", "tasks_status", "task_evidence", "task_evidence_status", "sub_task_id", "sub_task", "sub_task_status", "sub_task_date", "sub_task_start_date", "sub_task_end_date", "private_program", "task_deleted_flag", "sub_task_deleted_flag", "project_terms_and_condition", "task_remarks", "project_completed_date", "solution_id", "designation","project_remarks","project_evidence","channel","board_name","organisation_name","organisation_id","state_externalid", "school_externalid", "cluster_externalid", "block_externalid", "district_externalid"]
projects_df_cols1=projects_df_cols.filter((projects_df_cols["state_externalid"].isNull())&(projects_df_cols["district_externalid"].isNull())&(projects_df_cols["block_externalid"].isNull())&(projects_df_cols["school_externalid"].isNull())&(projects_df_cols["cluster_externalid"].isNull()))
projects_df_cols2=projects_df_cols.filter((projects_df_cols["state_externalid"].isNotNull())|(projects_df_cols["district_externalid"].isNotNull())|(projects_df_cols["block_externalid"].isNotNull())|(projects_df_cols["school_externalid"].isNotNull())|(projects_df_cols["cluster_externalid"].isNotNull()))
print(projects_df_cols1.count())
print(projects_df_cols2.count())
projects_df_cols2=projects_df_cols2["project_id", "project_created_type", "project_title", "project_title_editable", "program_id", "program_externalId", "program_name", "project_duration", "project_last_sync", "project_updated_date", "project_deleted_flag", "area_of_improvement", "status_of_project", "createdBy", "project_description", "project_goal", "parent_channel", "project_created_date", "task_id", "tasks", "task_assigned_to", "task_start_date", "task_end_date", "tasks_date", "tasks_status", "task_evidence", "task_evidence_status", "sub_task_id", "sub_task", "sub_task_status", "sub_task_date", "sub_task_start_date", "sub_task_end_date", "private_program", "task_deleted_flag", "sub_task_deleted_flag", "project_terms_and_condition", "task_remarks", "project_completed_date", "solution_id", "designation","project_remarks","project_evidence","channel","board_name","organisation_name","organisation_id"]
projects_df_colsn = projects_df_cols2.join(dffff,["project_id"],how='left')
for column in [column for column in projects_df_colsn.columns
               if column not in projects_df_cols1.columns]:
    projects_df_cols1 = projects_df_cols1.withColumn(column, lit(None))
projects_df_final = projects_df_cols1.union(projects_df_colsn)
print(projects_df_final.show())
# projects_df_final=projects_df_final.select(["project_id", "project_created_type", "project_title", "project_title_editable", "program_id", "program_externalId", "program_name", "project_duration", "project_last_sync", "project_updated_date", "project_deleted_flag", "area_of_improvement", "status_of_project", "createdBy", "project_description", "project_goal", "parent_channel", "project_created_date", "task_id", "tasks", "task_assigned_to", "task_start_date", "task_end_date", "tasks_date", "tasks_status", "task_evidence", "task_evidence_status", "sub_task_id", "sub_task", "sub_task_status", "sub_task_date", "sub_task_start_date", "sub_task_end_date", "private_program", "task_deleted_flag", "sub_task_deleted_flag", "project_terms_and_condition", "task_remarks", "project_completed_date", "solution_id", "designation","project_remarks","project_evidence","channel","board_name","organisation_name","organisation_id","state_id","block_id","district_id","cluster_id","school_id"])
# projects_df_final.show()
# projects_ent_df_melt.unpersist()
projects_df_cols.unpersist()
# final_projects_df = projects_df_final
...
# final_projects_df = obs_sub_df_final
