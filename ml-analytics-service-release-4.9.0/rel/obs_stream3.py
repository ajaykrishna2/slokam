...
try:
    def getRelatedEntity(entityId):
        urlEntityRelated = config.get("ML_SURVEY_SERVICE_URL", "url") + "/" + config.get("ML_SURVEY_SERVICE_URL",
                                                                                         "entity_related_end_point") + str(
            entityId)
...
try:
    def getUserRoles(userId):
        urlUserRoles = config.get("ML_SURVEY_SERVICE_URL", "url") + "/" + config.get("ML_SURVEY_SERVICE_URL",
                                                                                     "user_profile_end_point") + str(
            userId)
        headersUserRoles = {
            'Content-Type': config.get("API_HEADERS", "content_type"),
            'Authorization': "Bearer " + config.get("API_HEADERS", "authorization"),
...
try:
    ...
    class implementation:
        def buildnode(self, ob, crit, parent, ansCriteriaId):
            node1 = node()
            node1.type = ob['type']
                ...
                for pri in crit:
                    if ob['criteria']:
                        for cri in ob['criteria']:
                            if (str(cri['criteriaId']) == str(pri['parentCriteriaId'])) & (
                                    str(pri['_id']) == ansCriteriaId):
                                criObj = {}
                                criObj['type'] = 'criteria'
                                criObj['externalId'] = str(pri['_id'])
                                criObj['name'] = ''
    ...
    def obj_creation(obSub):
        ...
        if obSub['status'] == 'completed':
            ...
            if 'isAPrivateProgram' in obSub:
                ...
                try:
                    if obSub['metaInformation']['gpsLocation']:
                        gpsLocation = (obSub['metaInformation']['gpsLocation']).split(',')
                ...
                #                                                    {"$in": entitiesArrIds}},
                #                                               {"registryDetails.code": {"$in": entitiesArrIds}}]},
                #                                      {"metaInformation.name": 1, "entityType": 1,
                #                                       "registryDetails": 1}):
                ...
                try:
                    if obSub["userProfile"]:
                        if "userLocations" in obSub["userProfile"] and obSub["userProfile"]["userLocations"]:
                            # print(obSub["userProfile"]["userLocations"])
                            # print(next(x for x in obSub["userProfile"]["userLocations"]))
                            type1= [d['type']+"Name" for d in obSub["userProfile"]["userLocations"] if 'type' in d]
                            Namee= [d['name'] for d in obSub["userProfile"]["userLocations"] if 'name' in d]
                            type2 = [d['type'] + "ExternalId" for d in obSub["userProfile"]["userLocations"] if 'type' in d]
                            idd = [d['id'] for d in obSub["userProfile"]["userLocations"] if 'id' in d]
                            type3 = [d['type'] + "code" for d in obSub["userProfile"]["userLocations"] if'type' in d]
                            code = [d['code'] for d in obSub["userProfile"]["userLocations"] if 'code' in d]

                            res1 = dict(zip(type1, Namee))
                            res2 = dict(zip(type2,idd))
                            res3 = dict(zip(type3,code))
                            ent1={**res1, **res2,**res3}
                            # print(ent)
                            # stateName = ent["stateName"]
                            # stateExternalId = ent["stateExternalId"]
                            # blockName = ent["blockName"]
                            # blockExternalId = ent["blockExternalId"]
                            # districtName = ent["districtName"]
                            # districtExternalId = ent["districtExternalId"]
                            # print(ent["clusterName"])
                            # # clusterName = ent["clusterName"]
                            # # clusterExternalId = ent["clusterExternalId"]
                            # userSchool=ent["schoolExternalId"]
                            # userSchoolName = ent["schoolName"]
                            # userSchoolUDISE= ent["schoolcode"]
                            dic={}
                            for ent in obSub["userProfile"]["userLocations"]:
                                dic[f'{ent["type"]}_code']= ent["code"]
                                dic[f'{ent["type"]}_name'] = ent["name"]
                                dic[f'{ent["type"]}_externalId'] = ent["id"]
                            print(dic)
                            if "state_name" in dic:
                                stateName = dic["state_name"]
                                stateExternalId = dic["state_externalId"]
                            else:
                                stateName = ''
                                stateExternalId = ''
                            if "block_name" in dic:
                                blockName = dic["block_name"]
                                blockExternalId = dic["block_externalId"]
                            else:
                                blockName = ''
                                blockExternalId = ''
                            if "district_name" in dic:
                                districtName = dic["district_name"]
                                districtExternalId = dic["district_externalId"]
                            else:
                                districtName = ''
                                districtExternalId = ''
                            print(dic)
                            if dic["cluster_name"] is not None:
                                clusterName = dic["cluster_name"]
                                clusterExternalId = dic["cluster_externalId"]
                            else:
                                clusterName = ''
                                clusterExternalId = ''
                            if dic["school_name"] is not None:
                                userSchool=dic["school_externalId"]
                                userSchoolName = dic["school_name"]
                                userSchoolUDISE= dic["school_code"]
                            else:
                                userSchool = ''
                                userSchoolName = ''
                                userSchoolUDISE = ''
                            ...
                            #     ent["type"]+"Name"==ent["name"]
                                ...
                                # print(type(ent["type"]))
                                # [ent["type"]+"Name"]=ent["name"]
                                # print(globals()[ent["type"]+"Name"])
                                # print(stateName)
                                # if ent["type"] == "state":
                                #     stateName = ent["name"]
                                #     stateExternalId = ent["id"]
                                #
                                # if ent["type"] == "block":
                                #     blockName = ent["name"]
                                #     blockExternalId = ent["id"]
                                #
                                # if ent["type"] == "district":
                                #     districtName = ent["name"]
                                #     districtExternalId = ent["id"]
                                #
                                # if ent["type"] == "cluster":
                                #     clusterName = ent["name"]
                                #     clusterExternalId = ent["id"]
                                #
                                # if ent["type"] == "school":
                                #     userSchool=ent["id"]
                                #     userSchoolName = ent["name"]
                                #     userSchoolUDISE= ent["code"]
                        ...
                        if "framework" in obSub["userProfile"] and obSub["userProfile"]["framework"]:
                            if "board" in obSub["userProfile"]["framework"] and len(
                                    obSub["userProfile"]["framework"]["board"]) > 0:
                ...
                if obsAppName == config.get("ML_APP_NAME", "survey_app"):
                                            ...
                                            if userEntityRelated:
                                                ...
                                                if userEntityRelatedResultKeyCheck == True:
                                                    if userEntityRelated["result"]:
                                                        if (userEntityRelated["result"][
                                                                "entityType"] == "district") or (
                                                                userEntityRelated["result"][
                                                                    "entityType"] == "block") or (
                                                                userEntityRelated["result"][
                                                                    "entityType"] == "cluster") or (
                                                                userEntityRelated["result"]["entityType"] == "state"):
                                                            roleObj['user_' + userEntityRelated["result"][
                                                                "entityType"] + 'Name'] = \
                                                                userEntityRelated["result"]["metaInformation"]["name"]
                                                            roleObj[
                                                                userEntityRelated["result"][
                                                                    "entityType"] + '_externalId'] = \
                                                                userEntityRelated["result"]["metaInformation"][
                                                                    "externalId"]
                                                        if userEntityRelated["result"]["entityType"] == "school":
                                                            roleObj['user_schoolName'] = \
                                                                userEntityRelated["result"]["metaInformation"]["name"]
                                                            roleObj['user_schoolId'] = str(
                                                                userEntityRelated["result"]["metaInformation"]["id"])
                                                            roleObj['user_schoolUDISE_code'] = \
                                                                userEntityRelated["result"]["metaInformation"][
                                                                    "externalId"]
                                                        for usrEntityData in userEntityRelated["result"][
                                                            "relatedEntities"]:
                                                            if (usrEntityData['entityType'] == "district") or (
                                                                    usrEntityData['entityType'] == "block") or (
                                                                    usrEntityData['entityType'] == "cluster") or (
                                                                    usrEntityData['entityType'] == "state"):
                                                                roleObj[
                                                                    'user_' + usrEntityData['entityType'] + 'Name'] = \
                                                                    usrEntityData['metaInformation']['name']
                                                                roleObj[usrEntityData['entityType'] + '_externalId'] = \
                                                                    usrEntityData['metaInformation']['externalId']
                                                            if usrEntityData['entityType'] == "school":
                                                                roleObj['user_schoolName'] = \
                                                                    usrEntityData["metaInformation"]["name"]
                                                                roleObj['user_schoolId'] = str(
                                                                    usrEntityData["metaInformation"]["id"])
                                                                roleObj['user_schoolUDISE_code'] = \
                                                                    usrEntityData["metaInformation"]["externalId"]
                    ...
                    # print(roleObj["user_schoolUDISE_code"])
                ...
                if 'answers' in obSub.keys():
                    ...
                    for ans in answersArr:
                        def sequenceNumber(externalId, answer, answerSection, solutionObj):
                            try:
                                for num in range(
                                        len(solutionObj['questionSequenceByEcm'][answer['evidenceMethod']][
                                                answerSection])
                                ):
                                    # print(num)
                                    # print(solutionObj['questionSequenceByEcm'])
                                    # print(answer['evidenceMethod'])
                                    # print([answerSection][num])
                                    # print(solutionObj['questionSequenceByEcm'])
                                    # print(externalId)

                                    if solutionObj['questionSequenceByEcm'][answer['evidenceMethod']][answerSection][
                                        num] == externalId:
                                        # print(num+1)
                                        # print(num)
                                        return num + 1
                        ...
                        def creatingObj(
                            ...
                            if entityLatitudeCreateObjFn and entityLongitudeCreateObjFn:
                                ...
                                if len(answerGpsLocation) > 0:
                                    ...
                                    observationSubQuestionsObj[
                                        'location_validated_with_geotag'] = 'gps location not found for question'
                                    observationSubQuestionsObj['distance_in_meters'] = ''
                            else:
                                observationSubQuestionsObj[
                                    'location_validated_with_geotag'] = 'gps location not found for school'
                                observationSubQuestionsObj['distance_in_meters'] = ''
                            ...
                            observationSubQuestionsObj[entityType + 'ExternalId'] = obSub['entityInformation'][
                                'externalId']
                            ...
                            try:
                                observationSubQuestionsObj['administrationTypes'] = obSub['entityInformation'][
                                    'administrationTypes']
                            except KeyError:
                            ...
                            # print(obSub['solutionInfo'])
                            # print(obSub['programInfo'])
                            # print(obSub.keys())

                            try:
                                if 'programInfo' in obSub:
                                    observationSubQuestionsObj['programName'] = obSub['programInfo']['name']
                                    observationSubQuestionsObj['programDescription'] = obSub['programInfo'][
                                        'description']
                            ...
                            try:
                                for cri in obSub['criteria']:
                                    observationSubQuestionsObj['criteriaExternalId'] = cri['externalId']
                                    observationSubQuestionsObj['criteriaName'] = cri['name']
                                    observationSubQuestionsObj['criteriaDescription'] = cri['description']
                            except KeyError:
                                observationSubQuestionsObj['criteriaExternalId'] = ''
                                observationSubQuestionsObj['criteriaName'] = ''
                                observationSubQuestionsObj['criteriaDescription'] = ''
                            ...
                            # if 'evidences' in obSub:
                            #     # print(evidencesArr)
                            #     for eviCQ in evidencesArr:
                            #         # print(eviCQ)
                            #         try:
                            ...
                            # print(obSub['solutionInfo'].keys())
                            # print(observationSubQuestionsObj)
                            # for evi in obSub["evidences"].values():
                            #     print(evi.keys())
                            ...
                            for q_data in solutionObj['questionSequenceByEcm'].values():
                                for secq in q_data:
                                    observationSubQuestionsObj['section'] = secq
                            ...
                            if solutionObj:
                                ...
                                observationSubQuestionsObj['questionSequenceByEcm'] = sequenceNumber(quesexternalId,
                                                                                                     answer,
                                                                                                     observationSubQuestionsObj[
                                                                                                         'section'],
                                                                                                     solutionObj)
                            ...
                            try:
                                if solutionObj['scoringSystem'] == 'pointsBasedScoring':
                                    ...
                                    try:
                                        observationSubQuestionsObj['totalpercentage'] = obSub[
                                    ...
                                    try:
                                        observationSubQuestionsObj['pointsBasedScoreInParent'] = answer[
                            ...
                            # print(obSub['observationInformation'])
                            ...
                            # print(obSub['criteria'])
                            # print(obSub['criteriaLevelReport'])
                            # print(obSub.keys())
                            # for cri in obSub['criteriaLevelReport'].values():
                            #     print(cri)
                            ...
                            if ans['responseType'] == 'matrix':
                                ...
                                try:
                                    for cri in obSub['criteria']:
                                        observationSubQuestionsObj['instanceParentCriteriaExternalId'] = cri[
                                            'externalId']
                                        observationSubQuestionsObj['instanceParentCriterianame'] = cri['name']
                                except KeyError:
                                    observationSubQuestionsObj['instanceParentCriteriaExternalId'] = ''
                                    observationSubQuestionsObj['instanceParentCriterianame'] = ''

                                # for cri in obSub['criteria']:
                                #     observationSubQuestionsObj['instanceParentCriteriaExternalId'] = cri['externalId']
                                #     oobservationSubQuestionsObj['instanceParentCriteriaExternalId'] = cri['name']
                                # for critQuesInst in criteriaQuestionsCollec.find({'_id': ObjectId(ans["criteriaId"])}):
                                #     observationSubQuestionsObj['instanceParentCriteriaExternalId'] = critQuesInst[
                                #         'externalId']
                                ...
                                observationSubQuestionsObj['instanceParentExternalId'] = ans['externalId']
                            ...
                            for domain in obSub['themes']:
                                ...
                                # for cri in obSub['criteria']:
                                #     for dom in domain['criteria']:
                                #         if cri['parentCriteriaId']==dom['criteriaId']:
                                #             li['cri_id']=cri['_id']
                                #             li['parentCriteriaId']=cri['parentCriteriaId']
                                #             da.insert(
                                #
                                #             # if da:
                                #             #     domain['cri']=da+li
                                #             # else:
                                #             #     domain['cri'] =da.insert(li)
                                #
                                # print(domain)

                                # print(domain)
                                # print(parent)
                                # print(domObj['externalId'])
                                # print(answer['criteriaId'])

                                try:
                                    if domain['criteria']:
                                        # print(domain['criteria'])
                                        domObj['theme'] = builder.buildnode(domain, cri, parent,
                                                                            str(answer['criteriaId']))

                                except KeyError:
                                    domObj['theme'] = builder.buildnode(domain, cri, parent, str(answer['criteriaId']))
                                # print(domObj['theme'])
                                                        ...
                                                        if str(criteria["_id"]) == str(answer['criteriaId']):
                                                            ...
                                                            try:
                                                                ...
                                                                for labelValue in levelArray:
                                                                    if (str((criteria['score'])) == labelValue[
                                                                        'level']):
                            ...
                            for themes in domainArr:
                                ...
                                for st in themes["theme"]:
                                    if (st["type"] == "criteria") and (
                                            observationSubQuestionsObj['criteriaId'] == str(st["externalId"])):
                                            ...
                                            for submTheme in obSub["themes"]:
                                                if submTheme["externalId"] == themes['externalId']:
                                                    observationSubQuestionsObj['domainLevel'] = submTheme[
                                                        "pointsBasedLevel"]
                                                    observationSubQuestionsObj['domainScore'] = submTheme[
                                                        "scoreAchieved"]
                                        except KeyError:
