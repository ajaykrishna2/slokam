        ...
        if obSub['status'] == 'completed':
            ...
            for ans in answersArr:
                for ques in questionsCollec.find({'_id': ObjectId(ans['qid'])}):
                    ans['externalId']=(ques['externalId'])
                    print(ans)
                    # li.append(ans)
            # print(li)
            ...
            if 'isAPrivateProgram' in obSub:
                ...
                for answer in answersArr:
                    ...
                    try:
                        surveySubQuestionsObj['questionExternalId'] = str(answer['qid'])
                        surveySubQuestionsObj['questionName'] = answer['payload']['question'][0]
                        # for name in answer['payload']['question']:
                        #     print(name)
                        #     surveySubQuestionsObj['questionName'] = name
                        # print(surveySubQuestionsObj['questionName'])
