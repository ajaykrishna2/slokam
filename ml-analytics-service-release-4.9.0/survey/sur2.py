...
try:
    ...
    async def surveyFaust(consumer):
        async for msg in consumer:
            ...
            print(msg_data["_id"])
            if 'isAPrivateProgram' in msg_data:
                surveySubQuestionsArr = []
                completedDate = str(msg_data['completedDate'])
                createdAt = str(msg_data['createdAt'])
                updatedAt = str(msg_data['updatedAt'])
                print(updatedAt)
                evidencesArr = [v for v in msg_data['evidences'].values()]
                evidence_sub_count = 0
                rootOrgId = None
                try:
                    if msg_data["userProfile"]:
                        if "rootOrgId" in msg_data["userProfile"] and msg_data["userProfile"]["rootOrgId"]:
                            rootOrgId = msg_data["userProfile"]["rootOrgId"]
                            print(rootOrgId)
                except KeyError:
                    pass
                if 'answers' in msg_data.keys():
                    answersArr = [v for v in msg_data['answers'].values()]
                    for ans in answersArr:
                        try:
                            if len(ans['fileName']):
                                evidence_sub_count = evidence_sub_count + len(ans['fileName'])
                            ...
                            pass
                    print(evidence_sub_count)
                    surveySubQuestionsObj = {}
                    try:
                        surveySubQuestionsObj['appName'] = msg_data["appInformation"]["appName"].lower()
                    except KeyError:
                        surveySubQuestionsObj['appName'] = config.get("ML_APP_NAME", "survey_app")

                    surveySubQuestionsObj['surveySubmissionId'] = str(msg_data['_id'])

                    surveySubQuestionsObj['createdBy'] = msg_data['createdBy']

                    try:
                        surveySubQuestionsObj['isAPrivateProgram'] = msg_data['isAPrivateProgram']
                    except KeyError:
                        surveySubQuestionsObj['isAPrivateProgram'] = True

                    try:
                        surveySubQuestionsObj['programExternalId'] = msg_data['programExternalId']
                    except KeyError:
                        surveySubQuestionsObj['programExternalId'] = None
                    try:
                        surveySubQuestionsObj['programId'] = str(msg_data['programId'])
                    except KeyError:
                        surveySubQuestionsObj['programId'] = None
                    try:
                        surveySubQuestionsObj['totalScore'] = msg_data['pointsBasedMaxScore']
                    except KeyError:
                        surveySubQuestionsObj['totalScore'] = ''
                    try:
                        surveySubQuestionsObj['scoreAchieved'] = msg_data['pointsBasedScoreAchieved']
                    except KeyError:
                        surveySubQuestionsObj['scoreAchieved'] = ''
                    surveySubQuestionsObj['solutionExternalId'] = msg_data['solutionExternalId']
                    surveySubQuestionsObj['surveyId'] = str(msg_data['surveyId'])
                    surveySubQuestionsObj['completedDate'] = completedDate
                    surveySubQuestionsObj['createdAt'] = createdAt
                    surveySubQuestionsObj['updatedAt'] = updatedAt
                    # try:
                    #     surveySubQuestionsObj['totalpercentage'] = msg_data['pointsBasedPercentageScore']
                    print(surveySubQuestionsObj)
                    for ans in answersArr:
                        def sequenceNumber(externalId, answer):
                            for solu in solutionsCollec.find({'_id': ObjectId(msg_data['solutionId'])}):
                                section = [k for k in solu['sections'].keys()]
                                # parsing through questionSequencebyecm to get the sequence number
                                try:
                                    for num in range(
                        ...
                        def creatingObj(answer, quesexternalId, ans_val, instNumber, responseLabel):
                            ...
                            try:
                                ...
                                surveySubQuestionsObj['programId'] = None
                            try:
                                surveySubQuestionsObj['programName'] = msg_data['name']
                            except KeyError:
                                surveySubQuestionsObj['programName'] = None
                            if 'surveyInformation' in msg_data:
                                if 'name' in msg_data['surveyInformation']:
                                    surveySubQuestionsObj['surveyName'] = msg_data['surveyInformation']['name']
                                # else:
                                #     try:
                                #         for ob in surveyCollec.find({'_id': msg_data['surveyId']}, {'name': 1}):
                                #             surveySubQuestionsObj['surveyName'] = ob['name']
                                #     except KeyError:
                                #         surveySubQuestionsObj['surveyName'] = ''
                            surveySubQuestionsObj['solutionExternalId'] = msg_data['solutionExternalId']
                                ...
                                try:
                                    if solu['scoringSystem'] == 'pointsBasedScoring':
                                        ...
                                        try:
                                            ...
                                            surveySubQuestionsObj['scoreAchieved'] = ''
                                        try:
                                            surveySubQuestionsObj['totalpercentage'] = msg_data[
